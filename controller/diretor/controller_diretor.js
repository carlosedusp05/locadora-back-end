...
const buscarDiretorPeloId = async function(id){
    //Realizando uma cópia do objeto MESSAGE_DEFAULT, permitindo que as alterações desta função não interfiram em outras funções.
    let MESSAGE = JSON.parse(JSON.stringify(MESSAGE_DEFAULT))
    ...
    try {
        ...
        if( id != '' && id != null && id != undefined && !isNaN(id) && id > 0){
            ...
            let result = await filmeDAO.getSelectByIdDirectors(parseInt(id))
...
const inserirDiretor = async function(diretor, contentType){
    ...
    try {
        ...
        if(String(contentType).toUpperCase() == 'APPLICATION/JSON'){
                ...
                let result = await filmeDAO.setInsertDirectors(diretor)
    
                if(result){

                    //Chama a função para receber o ID gerado no BD
                    let lastIdDiretor = await filmeDAO.getSelectLastIdDirector()

                    if(lastIdDiretor){
                        //Adiciona no JSON de diretor Id que foi gerado pelo BD
...
const atualizarDiretor = async function(diretor, id, contentType){
    ...
    try {
        ...
        if(String(contentType).toUpperCase() == 'APPLICATION/JSON'){
                ...
                let validarId = await buscarDiretorPeloId(id)
                ...
                if(validarId.status_code == 200){
                    ...
                    let result = await filmeDAO.setUpdateDirectors(diretor)
...
const excluirDiretor = async function(id){
    ...
    try {
        ...
        if( id != '' && id != null && id != undefined && !isNaN(id) && id > 0){
            ...
            //Chama a função para filtrar pelo id
            let result = await filmeDAO.setDeleteDirectors(parseInt(id))
...
const validarDadosDiretor = async function (diretor) {
    ...
    if(diretor.biografia == '' || diretor.biografia == null || diretor.biografia == undefined || diretor.biografia.length > 100){
        MESSAGE.ERROR_REQUIRED_FIELDS.invalid_field = 'Atributo biografia inválido!'
        return MESSAGE.ERROR_REQUIRED_FIELDS //400

    }else if(diretor.biografia == '' || diretor.biografia == null || diretor.biografia == undefined || diretor.biografia.length > 2000){
        MESSAGE.ERROR_REQUIRED_FIELDS.invalid_field = 'Atributo biografia inválido!'
        return MESSAGE.ERROR_REQUIRED_FIELDS //400

    }else if(diretor.data_nascimento == null || diretor.data_nascimento == undefined || diretor.data_nascimento.length != 10){
        MESSAGE.ERROR_REQUIRED_FIELDS.invalid_field = 'Atributo data nascimento inválido!'
        return MESSAGE.ERROR_REQUIRED_FIELDS //400

    }else if(diretor.data_falecimento == undefined && diretor.data_falecimento !== null && diretor.data_falecimento.length >10 ){
        MESSAGE.ERROR_REQUIRED_FIELDS.invalid_field = 'Atributo data falecimento inválido!'
        return MESSAGE.ERROR_REQUIRED_FIELDS //400

    }else if(diretor.is_ativo != true && diretor.is_ativo != false || diretor.is_ativo == undefined){
        MESSAGE.ERROR_REQUIRED_FIELDS.invalid_field = 'Atributo is ativo inválido!'
        return MESSAGE.ERROR_REQUIRED_FIELDS //400

    }else if(diretor.idade == undefined || diretor.idade == null || diretor.idade.length > 3 || isNaN(diretor.idade)){
        MESSAGE.ERROR_REQUIRED_FIELDS.invalid_field = 'Atributo idade inválido!'
        return MESSAGE.ERROR_REQUIRED_FIELDS //400

    }else if(diretor.foto == '' || diretor.foto == null || diretor.foto == undefined || diretor.foto.length > 200){
